...
`ACCENT` is the highlight color and `GROUND` is the page behind it. Each row offers three presets, and each preset carries a light and a dark value, so picking one changes both modes. Under each row is a hex field: type six digits and it takes over from the presets.
...
`MANUAL` looks up whatever city you type. `APPROX` asks [ipapi.co](https://ipapi.co) to place your IP address, which is city-level and needs no browser prompt. `PRECISE` uses the browser's location: Chrome asks once to grant the extension location access, Firefox prompts the way any site does. The `°C` / `°F` button converts. Forecasts come from [Open-Meteo](https://open-meteo.com), refreshed every 30 minutes and cached between; coordinates are rounded to about a kilometer before they are sent.

The location readout shows `LOCATING…` while a position is being resolved and `NO LOCATION` when the browser refuses or cannot supply one — pick another mode or grant access. On Firefox, switching to `APPROX` or `PRECISE` may also raise the browser's data-collection consent for location; declining it leaves the mode unchanged.
...
The hotkey is highlighted inside the label as `/g/mail`, and leaving it blank uses the label's first letter — when adding a link and when an edit row closes with the hotkey cleared. A hotkey claimed twice in one panel turns red, because only the first will fire. Panels themselves reorder by dragging them in the main rail.
...
The dropdown sets the default engine, and `SEARCH ON` / `OFF` controls whether `/` does anything. The field below adds your own engine: paste a search URL with `%s` where the query goes, like `https://example.com/search?q=%s`, and it appears in the list on prefix `c`. The template must start with `http://` or `https://`; until it does, the list simply does not offer it.
...
- `SAVED 2026.08.24 16:20` — when the setup this browser holds was last written to the profile, by any machine.
- `UNAVAILABLE` — no extension storage here, so profile sync cannot run.
- `PUSHED`, `PULLED` — a manual push or pull worked.
- `NOTHING STORED` — a pull found no setup in the profile yet.
- `FAILED` — the profile store could not be read; nothing was changed.
...
A button showing `···` is waiting on the network. Result lines clear themselves after a moment and the standing status returns.
