...
The Add-ons build also sets your home page, so browser start and new windows open the startpage unless Firefox is set to restore the previous session.
...
On Chrome, no prompt appears. Chromium gives extensions no way to ask for location at runtime, so the extension asks for the permission from install and `PRECISE` works immediately. A `NO PERMISSION` readout means location is blocked for the extension in Chrome's site settings (`chrome://settings/content/location`); lift the block and pick `PRECISE` again.
